...
- L38 example: Three demos — Demo 1: scaled dot-product attention heatmap using identity projections (W_Q=W_K=W_V=I) so attention reflects raw embedding similarity; "cat/chases/mouse" 4-d embeddings. Demo 2: pre-trained DnCNN denoiser via `denoisingNetwork('dncnn')` + `denoiseImage` (Image Processing Toolbox, no training needed), cameraman.tif at σ=25/50/75, PSNR table. Demo 3: train noise predictor on DigitDataset with ε-prediction loss (MSE, `regressionLayer`), 3 tunable params (n_per_class=200, sigma=0.30, n_epochs=20); subtract predicted ε to denoise; PSNR improvement. No HW for L38.
